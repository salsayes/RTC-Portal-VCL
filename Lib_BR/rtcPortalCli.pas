...
type
  ...
  published
    ...
    { Se todos os eventos implementados pelo componente precisarem acessar a GUI,
      para evitar verificar "Sender.inMainThread" e chamar Sender.Sync(Event)
      para cada evento, você pode definir a propriedade AutoSyncEvent como true,
      o que garante que qualquer evento atribuído a este componente seja
      chamado a partir da thread principal (sincronizado, quando necessário). }
    property AutoSyncEvents: boolean read FAutoSync write SetAutoSync
      default False;

{$IFDEF COMPRESS}
    { Use esta propriedade para definir qual nível de compressão você quer usar ao enviar dados deste
      Portal Client para outro Portal Client. O valor padrão de compressão é "rtcpCompNormal".
      Você pode usar níveis de compressão diferentes para cada Portal Client. Se o seu Portal Client
      precisar trabalhar com clientes que não oferecem suporte à compressão, será preciso usar "cNone".
      Observe que os dados serão comprimidos aqui e descomprimidos pelo Cliente receptor.
      Não será feita nenhuma descompressão/compressão no lado do Servidor. }
    property DataCompress: TrtcpCompressLevel read FCompress write SetCompress
      default rtcpCompNormal;
{$ENDIF}
    { Defina esta propriedade com um valor diferente de 0 para usar Criptografia automática
      com uma chave gerada aleatoriamente de "EncryptKey" bytes. Cada byte representa
      uma força de criptografia de 8 bits. Para uma criptografia forte de 256 bits, use 32 bytes. @html(<br><br>)

      A chave de criptografia final é combinada a partir de uma chave do lado do cliente
      e de uma chave recebida do servidor, enquanto o servidor decide a força da criptografia.
      Se o servidor não suportar Criptografia, os dados não serão criptografados,
      independentemente do valor usado em EncryptionKey. }
    property DataEncrypt: integer read FAutoEncrypt write SetAutoEncrypt
      default 0;

    { Se você precisa de uma conexão 100% segura, defina uma String de Chave Segura
      (combinação de letras, números e caracteres especiais) para cada
      par PServerModule/TRtcPortalCli, além do valor de EncryptionKey.
      O TRtcPortalCli só poderá se comunicar com um PServerModule se
      ambos usarem a mesma SecureKey. O valor padrão de SecureKey é
      uma String vazia (ou seja: nenhuma chave segura). @html(<br><br>)

      A SecureKey será usada no handshake de inicialização da criptografia,
      para criptografar a primeira combinação de chaves enviada pelo ClientModule.
      Como todos os outros pacotes de dados já são enviados usando algum tipo de criptografia,
      ao definir uma SecureKey você criptografa a única parte da chave que seria
      enviada sem criptografia especial. }
