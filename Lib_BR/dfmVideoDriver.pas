...
(* ** Esta unidade funciona como uma "interface" entre
  o RTC Host e o driver de vídeo DemoForge Mirage ** *)

{$INCLUDE rtcDefs.inc}
{$INCLUDE rtcPortalDefs.inc}

// Defina para usar a nova correção para o problema do MS RDP.
{.$DEFINE RTC_LBFIX}
...
const
  ...
  // O driver Mirage para o recurso TightVNC usa esta descrição:
  ...
  dmf_dfo_Ptr_Engage = 48; // ponteiro usado com este registro
...
type
  ...
  // *********************************************************************
  // NÃO ALTERE AS ESTRUTURAS/ DEVEM SER EXATAMENTE IGUAIS em kernel/app/video
  // *********************************************************************

  CHANGES_RECORD = packed record
    OpType: cardinal; // screen_to_screen, blit, novo cache, cache antigo
    rect, origrect: TRect;
    point: TPoint;
    color: cardinal; // número usado no array de cache
    refcolor: cardinal; // slot usado para passar dados de bitmap
  end;
  ...
  Esc_dmf_Qvi_IN = packed record
    ...
    connect_options: cardinal; // reservado. deve ser 0.
  end;
  ...
  TDriverCallParams = record // Estrutura interna, não use
  ...
  // classe base abstrata para região de acumulação
...
{$ENDIF}
  ...
  public
    ...
    // qryverinfo é suportado a partir da versão 1.1 (build 68)
...
{$ENDIF}
  if gdc <> 0 then
    fDriverConnected := ExtEscape(gdc, MAP1, 0, nil, sizeof(GETCHANGESBUF),
      @bufdata) > 0;
  // OBSERVAÇÃO: não necessariamente 0.
  // a maneira mais correta é: obter o contador atual de bufdata.buffer
  // e realizar a atualização inicial da tela inteira
  m_ReadPtr := 0;
end;
...
{$IFDEF RTC_LBFIX}
    // se (dm.dmBitsPerPel=24) então dm.dmBitsPerPel := 32;
{$ENDIF}
    // adicionar configurações 'Default.*' ao registro em hKeyProfile\mirror\device
    if dcp.Ex then
...
{$ELSE}
        ...
        @fill:
          cmp word[EDI+2], 0
          je @set // "X2" = 0? inicializar X1 e X2

          cmp word[EDI], AX
          jbe @x1ok         // "X1" <= x1 ? sem necessidade de atualizar
          mov word[EDI], AX // definir "X1"

        @x1ok:
          cmp word[EDI+2], DX
          jae @x2ok // "X2" >= x2? sem necessidade de atualizar
          mov word[EDI+2], DX

        @x2ok:
          mov byte[EDI+4], 1 // marcar "alterado"
          add EDI,5
          loop @fill     // laço
          jmp @finishing // concluído
...
begin
  ...
  begin
    for i := FTop2 to FBottom2 - 1 do
      if LinesX2[i].x2 > 0 then
        ...
        begin // primário alterado, comparar
          LinesX[i].chg := 0;
          if (LinesX2[i].x1 >= LinesX[i].x2) or (LinesX2[i].x2 <= LinesX[i].x1)
          then // sem tocar a região de varredura primária
          begin
            LinesX2[i].chg := 1; // fora da imagem primária, desenho normal
            inc(SecondLines);
          end
          else if (LinesX2[i].x1 < LinesX[i].x1) or
            (LinesX2[i].x2 > LinesX[i].x2) then
          // na lateral esquerda e/ou direita do primário
          begin
            LinesX2[i].chg := 2; // desenhar lado esquerdo ou direito
            inc(SecondLines);
          end
          else // igual ao primário ou dentro dele, aguardar refinamento primário
          begin
            LinesX2[i].x1 := 0;
            LinesX2[i].x2 := 0;
            LinesX2[i].chg := 0;
        ...
        else if LinesX2[i].chg > 0 then // pronto
          inc(SecondLines);
  end;
  ...
  begin
    ...
    FScanPass := ScanPass;
    // Processar apenas linhas novas na 1ª passagem ...
  ...
  begin
    ...
    if (ThirdLines > 0) or HaveLines then
      NowTime := GetTickCount - FScan2Delay * 2 // aguardando mais refinamento
    else
      NowTime := GetTickCount - FScan2Delay; // sem mais dados
          ...
          if LinesX2[i].chg > 0 then
            ...
            begin
              case LinesX2[i].chg of
                ...
                2: // anexado ao lado esquerdo e/ou direito do primário
                  begin
                    frst := False;
                    ...
                    LinesX2[i].x2 := 0;
                    LinesX2[i].chg := 0;
                    // se houver mais, será tratado na 2ª passagem
            ...
            begin
              // precisa aguardar o tempo passar
              Dec(SecondLines);
              inc(SecondSkip);
              if SecondLines = 0 then
              begin
  ...
  begin
    ...
    for i := FTop to FBottom - 1 do
      ...
      begin
        if LinesX2[i].x2 = 0 then
        begin
          LinesTime[i] := NowTime; // último
          LinesX2[i].x1 := LinesX[i].x1;
          LinesX2[i].x2 := LinesX[i].x2;
        end
        else
        begin
          if LinesX2[i].chg = 0 then // refinando novamente (espaço animado)
            LinesTime[i] := NowTime;
          if LinesX2[i].x1 > LinesX[i].x1 then
            LinesX2[i].x1 := LinesX[i].x1;
          if LinesX2[i].x2 < LinesX[i].x2 then
