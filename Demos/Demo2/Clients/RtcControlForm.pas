...
type
  ...
  private
    procedure UpdateUserButtons;
    function GetSelectedUser(var User: string): Boolean;
...
function TMainForm.GetSelectedUser(var User: string): Boolean;
  var
    node:PVirtualNode;
    data:PUserNodeData;
  begin
  Result:=False;
  node:=eUsers.GetFirstSelected;
  if Assigned(node) then
    begin
    data:=eUsers.GetNodeData(node);
    if Assigned(data) and (data^.UserName<>'') then
      begin
      User:=data^.UserName;
      Result:=True;
      end;
...
procedure TMainForm.UpdateUserButtons;
  var
    user:string;
  begin
  if GetSelectedUser(user) then
    begin
...
procedure TMainForm.eUsersGetText(Sender: TBaseVirtualTree;
...
procedure TMainForm.eUsersFreeNode(Sender: TBaseVirtualTree;
  Node: PVirtualNode);
  var
    data:PUserNodeData;
  begin
